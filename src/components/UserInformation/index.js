...
import { MOBILE_ACCESS } from './constants';
...
class UserInformation extends React.PureComponent {
  ...
  handleInitialization(user, businessInfo) {
    this.props.initialize({
      ...
      userId: user.userId,
      enrollmentId: businessInfo.enrollmentId,
    });
  }
  ...
  goToEnrollments() {
    ...
    history.push(`/enrollment/${businessInfo.enrollmentId}`);
  }
    ...
    return (
      <div>
        <form onSubmit={handleSubmit(this.handleSubmit)}>
          <AppBar title="User Information" showMenuIconButton={false} iconElementRight={this.renderActionButtons()} />
          <Card>
            ...
            <div style={styles.row}>{this.renderUserEmail()}</div>
            <div style={styles.row}>
  ...
  renderFirstName() {
    return <Field name="firstName" label="First name" component={FormTextField} disabled={this.state.disabled} />;
  }

  renderLastName() {
    return <Field name="lastName" label="Last name" component={FormTextField} disabled={this.state.disabled} />;
  }

  renderAccountStatus() {
    return <Field name="accountStatus" label="Status" component={FormTextField} disabled />;
  }

  renderRole() {
    return <Field name="type" label="Role" component={FormTextField} disabled />;
  }

  renderDOB() {
    return <Field name="dob" label="Date of Birth" component={FormDatePicker} disabled={this.state.disabled} />;
  }

  renderSSN() {
    return <Field name="ssn" label="SSN" component={FormTextField} disabled />;
  }

  renderEnrollment() {
    return <FlatButton label="Enrollment" style={styles.button} onClick={this.goToEnrollments} />;
  }
  ...
  renderUserEmail() {
    return <Field name="userEmail" label="User email" component={FormTextField} disabled={this.state.disabled} />;
  }

  renderUserId() {
    return <Field name="userId" label="User Id" component={FormTextField} disabled />;
  }
  ...
  renderMobileEnabled() {
    ...
    return <Field name="mobileEnabled" label="Mobile Enabled" component={FormTextField} disabled />;
  }
}
