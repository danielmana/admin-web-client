...
class BankingSubscriptionPlan extends React.PureComponent {
    ...
    handleInitialization(fundingSource, subscription, bLinked) {
        const periodicReload = find(fundingSource.autoReloads, (autoReload) => autoReload.bentoType.includes(PERIODIC_RELOAD));
        const lowBalanceReload = find(fundingSource.autoReloads, (autoReload) => autoReload.bentoType.includes(LOW_BALANCE_RELOAD));
        ...
        this.props.initialize({
            ...
            bankLinked: bLinked,
            lowBalanceReload: lowBalanceReload ? `${lowBalanceReload.active ? 'ON' : 'OFF'} (set on ${moment(lowBalanceReload.updatedDate).format('MM/DD/YYYY')}) - trigger: ${numeral(lowBalanceReload.threshold).format('$0,0.00')}` : 'N/A',
            periodicReload: periodicReload ? `${periodicReload.active ? 'ON' : 'OFF'} (Set on ${moment(periodicReload.updatedDate).format('MM/DD/YYYY')})` : 'N/A',
            lowBalanceAmount: lowBalanceReload ? `${numeral(lowBalanceReload.amount).format('$0,0.00')}` : 'N/A',
            periodicReloadAmount: periodicReload ? `${numeral(periodicReload.amount).format('$0,0.00')} / ${periodicReload.period} / ${TIME_INTERVAL_DICTIONARY[periodicReload.period][periodicReload.timeInterval]}` : 'N/A',
    ...
    renderLowBalanceReload() {
        return (
            <Field
                name="lowBalanceReload"
                label="Low balance reload"
    ...
    renderPeriodicReloadAmount() {
        return (
            <Field
                name="periodicReloadAmount"
                label="Periodic reload amount"
                component={FormTextField}
                disabled
            />
        );
    ...
    renderPseudoAccountNo() {
        return (
            <Field
                name="pseudoAccountNo"
                label="Pseudo account no"
                component={FormTextField}
                disabled
            />
        );
...
BankingSubscriptionPlan.propTypes = {
    bankingInfo: PropTypes.oneOfType([
        PropTypes.array,
        PropTypes.object,
    ]),
    subscriptionInfo: PropTypes.oneOfType([
        PropTypes.array,
        PropTypes.object,
    ]),
