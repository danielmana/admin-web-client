...
import { TRANSACTION_DETAIL_PROPERTIES } from 'components/TableRowTransactionDetails/constants';
import { capitalizeFirstLetter } from 'utils/helpers';


class TableRowTransactionDetails extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
    ...
    renderHistory() {
        ...
        return (
            ...
            >
                {map(history, this.renderHistoryItem)}
            </div>
        );
    }
    ...
    renderDetails() {
        ...
        return (
            <div
                style={{ display: 'flex', padding: '10px 0', flexDirection: 'column' }}
            >
                {map(omit(firstHistoryItem, TRANSACTION_DETAIL_PROPERTIES), this.renderHistoryItemField)}
            </div>
        );
    }
    ...
    renderHistoryItem(item, index) {
        return (
            <div
                ...
                key={index}
            >
                {map(pick(item, TRANSACTION_DETAIL_PROPERTIES), this.renderHistoryItemField)}
            </div>
        );
    }
