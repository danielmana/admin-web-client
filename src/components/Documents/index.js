...
import DialogUploadDocument from 'containers/EnrollmentPage/DialogUploadDocument/index';
...
class Documents extends React.PureComponent {
  ...
  render() {
    return (
      <div>
        <AppBar title="Documents" showMenuIconButton={false} iconElementRight={this.uploadAction()} />
        <Card>{this.renderUploadedDocuments()}</Card>
  ...
  uploadAction() {
    return <FlatButton label="Upload" onClick={this.handleDialogUpload} />;
  }
  ...
  renderDialogUpload() {
    ...
    return <DialogUploadDocument visible={this.state.open} onCancel={this.onDialogCancel} businessID={businessID} />;
  }

  renderUploadedDocuments() {
    const { documents } = this.props;
    return <List>{map(documents, this.renderDocument)}</List>;
  }
  ...
  renderDocumentDownloadText(href, fileName) {
    return <div style={{ width: '100%', overflow: 'hidden', textOverflow: 'ellipsis' }}>{fileName}</div>;
  }

  renderDocumentDownloadButton(href, fileName) {
    return (
      <IconButton href={href} target="_self" download={fileName}>
