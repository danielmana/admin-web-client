...
class InitiateCredit extends React.PureComponent {
  ...
  render() {
    return (
      <div>
        <AppBar title="Initiate Credit" showMenuIconButton={false} iconElementRight={this.renderCreditButton()} />
        <Card>
  ...
  renderConfirmDialog() {
    const { setInitiateCreditDebit, amount, initiateCredit, reason, description, businessId } = this.props;
    const data = {
      businessId,
      amount,
      reason,
  ...
  renderCreditButton() {
    return <FlatButton label="Credit" onClick={this.handleOpenCreditDialog} />;
  }

  renderInitiateCreditOptions() {
    return <Field name="initiateCredit" label="Initiate Credit" options={CREDIT_OPTIONS} component={FormSelectField} />;
  }

  renderInitiateCreditDialog() {
    const { initiateCredit } = this.props;
    const actions = [
      <FlatButton label="Cancel" onClick={this.handleCloseCreditDialog} />,
  ...
  renderAmount() {
    return <Field name="amount" label="Amount" component={FormTextField} />;
  }

  renderCourtesyReasons() {
    return <Field name="reason" label="Enter reason" options={COURTESY_CREDIT_REASONS} component={FormSelectField} />;
  }

  renderDescription() {
    return <Field name="description" label="Description" component={FormTextField} />;
  }
}
