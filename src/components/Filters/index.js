...
class Filters extends React.PureComponent {
  ...
  render() {
    ...
    if (simpleSearch) {
      return <form onSubmit={handleSubmit((values) => onSubmit(values))}>{this.renderActions()}</form>;
    }

    return (
      <form onSubmit={handleSubmit((values) => onSubmit(values))}>
        <Card style={styles.root} initiallyExpanded={false}>
          <CardText style={styles.content} expandable>
            {children}
          </CardText>

          <CardHeader title="Advanced Search" actAsExpander showExpandableButton />
        </Card>
        {this.renderActions()}
      </form>
    );
  ...
  renderActions() {
    ...
    return (
      <CardActions style={styles.actions} expandable>
        <RaisedButton type="submit" label="Search" disabled={pristine || submitting} secondary />
        <FlatButton
          type="button"
          label="Clear"
